    ...
    public void run ()
    {
        while (!terminated())
        {
            try
            {
                ...
                SurgeryUnit.Prep.broken();
                SurgeryUnit.Prep.cancel();

                if (!SurgeryUnit.PreparationQ.isEmpty())
                    interrupted_service = true;

                hold(failedTime);

                SurgeryUnit.PreparationRoomFailedTime += failedTime;
                SurgeryUnit.Prep.fixed();

                if (interrupted_service)
                    SurgeryUnit.Prep.activateAt(SurgeryUnit.Prep.serviceTime()
                            + currentTime());
                else
